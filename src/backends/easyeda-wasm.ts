...
function boardToRouterInput(board: RoutingBoard, routeLayers: readonly string[]) {
      ...
      try {
        ...
        return {
          ...
          metrics: {
            ...
            routedNetCount: new Set(copper.tracks.map((track) => track.net)).size,
            trackLengthMm: length,
            viaCount: copper.vias.length,
            details: { progress, routability: Number.isFinite(routability) ? routability : null },
          },
        }
      } catch (error) {
        return {
