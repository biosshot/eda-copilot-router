...
import { materializeRoutingStackup, routingStackPlanes } from "../core/stackup.js"
import {
...
function referenceCopper(board: RoutingBoard, program: RoutingProgram) {
  ...
  for (const plane of routingStackPlanes(board, program.stack)) {
    for (const net of plane.nets) output.push({ net, layer: plane.layer })
  }
...
function applyImpedanceConstraint(
  ...
  const dedicatedPlaneLayers = new Set(routingStackPlanes(board, program.stack)
    .map((plane) => plane.layer))
  const allowed = (base.allowedLayers ?? board.layers.map((layer) => layer.name))
    .filter((layer) => !dedicatedPlaneLayers.has(layer))
...
export function compileRoutingRules(
  ...
  const stackPlanes = routingStackPlanes(board, program.stack)
  ...
  for (const plane of stackPlanes) {
    for (const net of plane.nets) checkNet(net)
  }
  if (stackPlanes.length && (program.operation === "route" || program.operation === "all")) required.add("zones")
  if (stackPlanes.length && program.operation === "copper") diagnostics.push(diagnostic(
    "DSL_STACK_PLANE_ROUTING_REQUIRED",
    "stack-declared full/split planes require runRouting() or runAll() because they are generated by the native KRT plane engine.",
  ))
  ...
  for (const plane of program.planes) {
    ...
    const dedicated = new Set(stackPlanes.map((item) => item.layer))
    const conflicts = selectedLayers(board, plane.layers).filter((layer) => dedicated.has(layer))
    if (conflicts.length) diagnostics.push(diagnostic(
      "DSL_STACK_PLANE_CONFLICT",
      `plane() cannot target stack dedicated-plane layer(s): ${conflicts.join(", ")}.`,
      { layers: conflicts, net: plane.net },
    ))
