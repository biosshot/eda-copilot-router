...
function configuredPath(name) {
  ...
  if (!value) throw new TypeError(`Missing ${name}; invoke this runner through a case run.mjs`)
  return resolve(value)
}

const fixturePcb = configuredPath("COPILOT_ROUTER_E2E_FIXTURE_PCB")
const fixtureProject = process.env.COPILOT_ROUTER_E2E_FIXTURE_PROJECT
  ...
  : undefined
const dslPath = configuredPath("COPILOT_ROUTER_E2E_DSL")
const suiteName = process.env.COPILOT_ROUTER_E2E_SUITE
if (!suiteName) throw new TypeError("Missing COPILOT_ROUTER_E2E_SUITE")
const caseName = process.env.COPILOT_ROUTER_E2E_CASE
if (!caseName) throw new TypeError("Missing COPILOT_ROUTER_E2E_CASE")
