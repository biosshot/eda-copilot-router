...
const genericRunner = join(routerDirectory, "tests", "e2e", "powerbank", "run.mjs")
...
async function runCase(entry, options, signal) {
  const arguments_ = [genericRunner, "--profile", options.profile, "--max-candidates", String(options.maxCandidates)]
  if (options.runId) arguments_.push("--run-id", options.runId)
  const environment = {
    ...process.env,
    COPILOT_ROUTER_E2E_SUITE: "kicad-routing-tools",
    COPILOT_ROUTER_E2E_CASE: entry.id,
    COPILOT_ROUTER_E2E_FIXTURE_PCB: join(testDirectory, entry.board),
    COPILOT_ROUTER_E2E_DSL: join(testDirectory, entry.dsl),
    COPILOT_ROUTER_E2E_REQUIRE_UNROUTED: "0",
  }
  if (entry.project) environment.COPILOT_ROUTER_E2E_FIXTURE_PROJECT = join(testDirectory, entry.project)
  else delete environment.COPILOT_ROUTER_E2E_FIXTURE_PROJECT
