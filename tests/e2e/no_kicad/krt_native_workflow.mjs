...
try {
  ...
  const patchProbe = [
    ...
    `assert length_matching.find_nets_matching_patterns(['DATA0'],[${JSON.stringify(rawProbeTokenByName["DATA[0]"])}])==[]`,
    `assert length_matching.find_nets_matching_patterns(['DATA[0]'],[${JSON.stringify(rawProbeTokenByName["DATA[0]"])},${JSON.stringify(rawProbeTokenByName["DATA[[]0]"])}])==['DATA[0]']`,
    ...
    "partial_pcb=types.SimpleNamespace(nets={1:types.SimpleNamespace(name='DATA[0]')},pads_by_net={})",
    `case.assertRaisesRegex(RuntimeError,'absent from the PCB',net_queries.expand_net_patterns,partial_pcb,[${JSON.stringify(rawProbeTokenByName["DATA[0]"])},${JSON.stringify(rawProbeTokenByName["DATA[[]0]"])}])`,
